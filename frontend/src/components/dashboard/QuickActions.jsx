...
import { PlusCircle, Timer, Gamepad2, Smile, Moon } from 'lucide-react';
import { useNavigate } from 'react-router-dom';

export const QuickActions = ({ onOpenCheckIn, onOpenAddTask, onOpenSleepTimer }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-100 dark:border-slate-800 shadow-soft transition-colors duration-200">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        {/* Sleep & Rest Timer */}
        <button
          onClick={onOpenSleepTimer || (() => navigate('/focus?tab=sleep'))}
          className="flex items-center gap-3 p-3.5 rounded-2xl bg-slate-50/70 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700/60 hover:border-indigo-200 dark:hover:border-indigo-800 hover:bg-indigo-50/40 dark:hover:bg-indigo-950/30 text-slate-700 dark:text-slate-200 hover:text-indigo-700 dark:hover:text-indigo-300 transition-all text-left group cursor-pointer"
        >
          <div className="w-8 h-8 rounded-xl bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
            <Moon className="w-4 h-4" />
          </div>
          <div className="flex flex-col">
            <span className="text-xs font-bold">Sleep Timer</span>
            <span className="text-[10px] text-indigo-600 dark:text-indigo-400 font-semibold">-Burnout</span>
          </div>
        </button>
        ...
        <button
          ...
          className="col-span-2 flex items-center justify-between p-3 rounded-2xl bg-slate-50/70 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700/60 hover:border-purple-200 dark:hover:border-purple-800 hover:bg-purple-50/40 dark:hover:bg-purple-950/30 text-slate-700 dark:text-slate-200 hover:text-purple-700 dark:hover:text-purple-300 transition-all text-left group cursor-pointer"
        >
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-bold">Wellbeing Games & Breathing</span>
          </div>
          <span className="text-[10px] font-bold text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-950/50 px-2 py-0.5 rounded-lg border border-purple-200/50 dark:border-purple-800/40">
            Quick De-stress
          </span>
        </button>
      </div>
